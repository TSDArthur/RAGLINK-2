...
namespace RAGLINKCommons.RPlatform
{
    ...
    public partial class FormSummary : Form
    {
        ...
        private void FormPlatform_Load(object sender, EventArgs e)
        {
            ...
            if (errorCount > 0)
            {
                ...
                toolStripLabelMain.Text = "加载失败，存在" +
                    ...
                    infoCount.ToString() + "个消息。联系系统管理员获得技术支持。";
                if (!RPlatform.ProjectsManager.projectInfo.projectDebug)
                {
                    buttonLaunch.Enabled = false;
                }
            }
            else if (warningCount > 0)
            {
                ...
                toolStripLabelMain.Text = "加载成功，存在" +
                    warningCount.ToString() + "个警告，" +
                    infoCount.ToString() + "个消息。运行将会不稳定。";
                if (!RPlatform.ProjectsManager.projectInfo.projectDebug)
                {
                    buttonLaunch.Enabled = false;
                }
        ...
        private void ButtonLaunch_Click(object sender, EventArgs e)
        {
            ...
            RProxy.UserInterfaceSwap.debugMode = RPlatform.ProjectsManager.projectInfo.projectDebug;
            RProxy.UserInterfaceSwap.showFormController = RPlatform.ProjectsManager.projectInfo.projectDebug;
            new System.Threading.Thread((System.Threading.ThreadStart)delegate
            {
                Application.Run(new RProxy.FormController());
            }).Start();
