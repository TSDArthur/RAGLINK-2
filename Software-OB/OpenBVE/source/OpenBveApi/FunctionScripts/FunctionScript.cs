        ...
        public FunctionScript(HostInterface Host, string Expression, bool Infix)
        {
            ...
            for (int i = 0; i < Arguments.Length; i++)
            {
                double d; if (double.TryParse(Arguments[i], System.Globalization.NumberStyles.Float, Culture, out d))
                {
                    ...
                    {
                        Array.Resize<Instructions>(ref InstructionSet, InstructionSet.Length << 1);
                    }

                    InstructionSet[n] = Instructions.SystemConstant;
                    if (c >= Constants.Length)
                    {
                        Array.Resize<double>(ref Constants, Constants.Length << 1);
                    }
                ...
                else
                {
                    ...
                    switch (Arguments[i].ToLowerInvariant())
                    {
                        ...
                        case "~":
                            if (s < 1)
                            {
                                throw new System.InvalidOperationException(Arguments[i] + " requires at least 1 argument on the stack in function script " + Expression);
                            }
                        ...
                        case "<>":
                            if (s < 2)
                            {
                                throw new System.InvalidOperationException(Arguments[i] + " requires at least 2 arguments on the stack in function script " + Expression);
                            }
                        ...
                        case "fma":
                            if (s < 3)
                            {
                                throw new System.InvalidOperationException(Arguments[i] + " requires at least 3 arguments on the stack in function script " + Expression);
                            }
