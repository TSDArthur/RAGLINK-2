...
namespace TrainEditor
{
	public partial class formEditor : Form
	{
		...
		{
			// performance
			if (!SaveControlContent(textboxDeceleration, "Deceleration", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Performance.Deceleration))
			{
				return false;
			}

			if (!SaveControlContent(textboxCoefficientOfStaticFriction, "CoefficientOfStaticFriction", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Performance.CoefficientOfStaticFriction))
			{
				return false;
			}

			if (!SaveControlContent(textboxCoefficientOfRollingResistance, "CoefficientOfRollingResistance", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Performance.CoefficientOfRollingResistance))
			{
				return false;
			}

			if (!SaveControlContent(textboxAerodynamicDragCoefficient, "AerodynamicDragCoefficient", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Performance.AerodynamicDragCoefficient))
			{
				return false;
			}
			// Jerk
			if (!SaveControlContent(textboxJerkPowerUp, "JerkPowerUp", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Move.JerkPowerUp))
			{
				return false;
			}

			if (!SaveControlContent(textboxJerkPowerDown, "JerkPowerDown", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Move.JerkPowerDown))
			{
				return false;
			}

			if (!SaveControlContent(textboxJerkBrakeUp, "JerkBrakeUp", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Move.JerkBrakeUp))
			{
				return false;
			}

			if (!SaveControlContent(textboxJerkBrakeDown, "JerkBrakeDown", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Move.JerkBrakeDown))
			{
				return false;
			}

			if (!SaveControlContent(textboxBrakeCylinderUp, "BrakeCylinderUp", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Move.BrakeCylinderUp))
			{
				return false;
			}

			if (!SaveControlContent(textboxBrakeCylinderDown, "BrakeCylinderDown", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Move.BrakeCylinderDown))
			{
				return false;
			}
			// brake
			...
			if (!SaveControlContent(textboxBrakeControlSpeed, "BrakeControlSpeed", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Brake.BrakeControlSpeed))
			{
				return false;
			}
			// pressure
			if (!SaveControlContent(textboxBrakeCylinderServiceMaximumPressure, "BrakeCylinderServiceMaximumPressure", tabpagePropertiesOne, NumberRange.Positive, out Train.Pressure.BrakeCylinderServiceMaximumPressure))
			{
				return false;
			}

			if (!SaveControlContent(textboxBrakeCylinderEmergencyMaximumPressure, "BrakeCylinderEmergencyMaximumPressure", tabpagePropertiesOne, NumberRange.Positive, out Train.Pressure.BrakeCylinderEmergencyMaximumPressure))
			{
				return false;
			}

			if (!SaveControlContent(textboxMainReservoirMinimumPressure, "MainReservoirMinimumPressure", tabpagePropertiesOne, NumberRange.Positive, out Train.Pressure.MainReservoirMinimumPressure))
			{
				return false;
			}

			if (!SaveControlContent(textboxMainReservoirMaximumPressure, "MainReservoirMaximumPressure", tabpagePropertiesOne, NumberRange.Positive, out Train.Pressure.MainReservoirMaximumPressure))
			{
				return false;
			}

			if (!SaveControlContent(textboxBrakePipeNormalPressure, "BrakePipeNormalPressure", tabpagePropertiesOne, NumberRange.Positive, out Train.Pressure.BrakePipeNormalPressure))
			{
				return false;
			}
			// handle
			...
			if (!SaveControlContent(textboxPowerNotchReduceSteps, "PowerNotchReduceSteps", tabpagePropertiesOne, NumberRange.NonNegative, out Train.Handle.PowerNotchReduceSteps))
			{
				return false;
			}
			// cab
			if (!SaveControlContent(textboxX, "X", tabpagePropertiesTwo, NumberRange.Any, out Train.Cab.X))
			{
				return false;
			}

			if (!SaveControlContent(textboxY, "Y", tabpagePropertiesTwo, NumberRange.Any, out Train.Cab.Y))
			{
				return false;
			}

			if (!SaveControlContent(textboxZ, "Z", tabpagePropertiesTwo, NumberRange.Any, out Train.Cab.Z))
			{
				return false;
			}

			if (!SaveControlContent(textboxDriverCar, "DriverCar", tabpagePropertiesTwo, NumberRange.NonNegative, out Train.Cab.DriverCar))
			{
				return false;
			}
			// car
			if (!SaveControlContent(textboxMotorCarMass, "MotorCarMass", tabpagePropertiesTwo, NumberRange.Positive, out Train.Car.MotorCarMass))
			{
				return false;
			}

			if (!SaveControlContent(textboxNumberOfMotorCars, "NumberOfMotorCars", tabpagePropertiesTwo, NumberRange.Positive, out Train.Car.NumberOfMotorCars))
			{
				return false;
			}

			if (!SaveControlContent(textboxTrailerCarMass, "TrailerCarMass", tabpagePropertiesTwo, NumberRange.Positive, out Train.Car.TrailerCarMass))
			{
				return false;
			}

			if (!SaveControlContent(textboxNumberOfTrailerCars, "NumberOfTrailerCars", tabpagePropertiesTwo, NumberRange.NonNegative, out Train.Car.NumberOfTrailerCars))
			{
				return false;
			}
			...
			if (!SaveControlContent(textboxLengthOfACar, "LengthOfACar", tabpagePropertiesTwo, NumberRange.Positive, out Train.Car.LengthOfACar))
			{
				return false;
			}

			Train.Car.FrontCarIsAMotorCar = checkboxFrontCarIsMotorCar.Checked;
			if (!SaveControlContent(textboxWidthOfACar, "WidthOfACar", tabpagePropertiesTwo, NumberRange.Positive, out Train.Car.WidthOfACar))
			{
				return false;
			}

			if (!SaveControlContent(textboxHeightOfACar, "HeightOfACar", tabpagePropertiesTwo, NumberRange.Positive, out Train.Car.HeightOfACar))
			{
				return false;
			}

			if (!SaveControlContent(textboxCenterOfGravityHeight, "CenterOfGravityHeight", tabpagePropertiesTwo, NumberRange.Any, out Train.Car.CenterOfGravityHeight))
			{
				return false;
			}

			if (!SaveControlContent(textboxExposedFrontalArea, "ExposedFrontalArea", tabpagePropertiesTwo, NumberRange.Positive, out Train.Car.ExposedFrontalArea))
			{
				return false;
			}

			if (!SaveControlContent(textboxUnexposedFrontalArea, "UnexposedFrontalArea", tabpagePropertiesTwo, NumberRange.Positive, out Train.Car.UnexposedFrontalArea))
			{
				return false;
			}
			// device
			...
			if (!SaveControlContent(textboxDoorWidth, "DoorWidth", tabpagePropertiesTwo, NumberRange.NonNegative, out Train.Device.DoorWidth))
			{
				return false;
			}

			if (!SaveControlContent(textboxDoorMaxTolerance, "DoorMaxTolerance", tabpagePropertiesTwo, NumberRange.NonNegative, out Train.Device.DoorMaxTolerance))
			{
				return false;
			}
			// finish
