...
namespace OpenBve
{
	public partial class TrainMethods
	{
		...
		{
			SimCoreClient.TriggerAttachEvent += AttachToTrigger;
			SimCoreClient.EnableTriggerClientEvent += EnableTriggerClient;
			SimCoreClient.GetSimulatorStateEvent += GetSimulatorState;
			SimCoreClient.RestartSimulatorStateEvent += RestartSimulator;
			SimCoreClient.PauseSimulatorStateEvent += PauseSimulator;
		}
		...
		static public bool RestartSimulator(string executeFileName, string Args)
		{
			...
			try
			{
				...
				processManager.StartInfo.FileName = RAGLINKCommons.RPlatform.SettingsContent.platformPath + "\\" + executeFileName;
		...
		static public void PauseSimulator(bool dataValue)
		{
			try
			{
				if (Game.CurrentInterface == Game.InterfaceType.Normal && dataValue)
				{
					Game.CurrentInterface = Game.InterfaceType.Pause;
				}
				else if (Game.CurrentInterface == Game.InterfaceType.Pause && !dataValue)
				{
		...
		static private SimWorldTrigger.TriggerClient TriggerClient = new SimWorldTrigger.TriggerClient();

		static public void AttachToTrigger()
		{
			SimWorldTrigger.DeleteTriggerClienr(TriggerClient);
			TriggerClient = new SimWorldTrigger.TriggerClient(200, TriggerEvent);
			triggerClientID = SimWorldTrigger.AddTriggerClient(TriggerClient);
		}

		static public void EnableTriggerClient()
		{
			SimWorldTrigger.SetClientState(TriggerClient, false);
		}
		...
		static public void SetTrainPowerHandleUp()
		{
			try
			{
				...
				if (currentTrainInControl.Handles.EmergencyBrake.Actual)
				{
				...
				if (currentBrake > 0)
				{
					currentTrainInControl.ApplyNotch(0, true, -1, true);
				}
				else
				{
					currentTrainInControl.ApplyNotch(1, true, 0, true);
				}
		...
		static private void ConstSpeedController(bool ignoreReadLight, double constSpeed)
		{
			if (constSpeed >= 0)
			{
				try
				{
					...
					{
						if (GetNextSectionSignalDistance() / currentSpeed <= 15 && Math.Abs(currentSpeed / (Math.Abs(accSpeed) + 1e-5)) <= 30)
						{
							destSpeed = 0;
						}
						else
						{
							destSpeed = Math.Min(30 / 3.6, destSpeed);
						}
					}
					//
					...
					if (relativeSpeed > 0)
					{
						...
						if (accSpeed < 0)
						{
							SetTrainPowerHandleUp();
						}
						else if (accSpeed > 0)
						{
							if (Math.Abs(relativeSpeed / accSpeed) <= pid_k)
							{
								SetTrainPowerHandleDown();
							}
							else
							{
								SetTrainPowerHandleUp();
		...
		static public double GetCurrentTrackPosition()
		{
			double retValue = 0;
			try
			{
				retValue = currentTrainInControl.Cars[currentTrainInControl.DriverCar].FrontAxle.Follower.TrackPosition + 2.3;
			}
			catch (Exception) { };
			return retValue;
		}
		...
		static public string GetDestinationStationName()
		{
			...
			try
			{
				...
				if (Table.Stations.Length == 0 || destStationIndex - 1 < 0)
				{
					return errState;
				}
		...
		static public string GetNextStationName()
		{
			...
			try
			{
				...
				if (Table.Stations.Length == 0 || nextStationIndex >= Table.Stations.Length)
				{
					return errState;
				}
		...
		static public double GetPreStationDistance()
		{
			...
			try
			{
				...
				//
				int nextStationIndex = GetNextStationIndex();
				if (nextStationIndex == -1 || nextStationIndex - 1 < 0)
				{
					return errState;
				}
		...
		static public string GetNextStationArrialTime()
		{
			...
			try
			{
				...
				if (Hour == string.Empty || Hour == " " || Hour == "  ")
				{
					return errState;
				}
		...
		static public double GetStopPointDistance()
		{
			...
			try
			{
				...
				if ((GetNextStationDistance() <= 180 || GetPreStationDistance() <= 180) && retValue >= -180)
				{
					return retValue;
				}
		...
		static public double GetCurrentPowerRate()
		{
			...
			try
			{
				...
				for (int i = 0; i < carsNumber; i++)
				{
					...
					if (currentTrainInControl.Cars[i].Specs.IsMotorCar)
					{
						carWeight += v * 250;
					}
					else
